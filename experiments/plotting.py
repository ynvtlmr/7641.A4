...
def plot_value_grad(v, map_desc):
    fig, ax = plt.subplots()
    im = ax.imshow(v)
    ...
    if len(v) < 30:
        for i in range(len(v)):
            for j in range(len(v[i])):
                text = ax.text(j, i, (map_desc[i, j]).decode('utf-8'),
                               ha="center", va="center", color="w")

    fig.tight_layout()
    return watermark(plt)

def plot_value_q(v, q, map_desc, direction_map):
    fig, ax = plt.subplots()
    im = ax.imshow(v)

    fontsize = 10
    if map_desc.shape[0] > 4:
        fontsize = 5
    elif map_desc.shape[0] > 15:
        fontsize = 3
    elif map_desc.shape[0] > 30:
        fontsize = 1
    # Loop over data dimensions and create text annotations.
    actions = ['L','D','R','U']
    for i in range(len(map_desc)):
        for j in range(len(map_desc[0])):
            t = ""
            for k in range(len(q[i * len(map_desc[0]) + j])):
                t += ('{}: {}\n'.format(actions[k], round(q[i * len(map_desc[0]) + j][k], 2)))

            t += "{} : {}".format(actions[np.argmax(q[i * len(map_desc[0]) + j])],
                                  direction_map[np.argmax(q[i * len(map_desc[0]) + j])])
...
def plot_reward_and_delta_vs_steps(title, df, xlabel="Steps", ylabel="Reward"):
    ...
    lns1 = ax.plot(df.index.values, df['reward'], linewidth=1, label=ylabel, color='C0')

    ex_ax = ax.twinx()
    lns2 = ex_ax.plot(df.index.values, df['delta'], linewidth=1, label='Delta', color='C1')
...
def find_optimal_params(problem_name, base_dir, file_regex):
    ...
    for f in grid_files:
        ...
        ## ALWAYS PLOTS 0.9. UNCOMMENT TO FIND BEST. ##
...
def find_data_files(base_dir, params):
    ...
    for mdp in params:
        ...
        optimal_files = glob.glob(
            '{}/{}_{}_optimal.csv'.format(base_dir, mdp_params['name'], mdp_params['param_str']))
        episode_files = glob.glob(
            '{}/{}_{}_episode.csv'.format(base_dir, mdp_params['name'], mdp_params['param_str']))
...
def plot_data(data_files, envs, base_dir):
    for problem_name in data_files:
        for mdp in data_files[problem_name]:
            ...
            gamma = mdp_files['file'].split('_')[-1].split('.csv')[0]
            total_time = df['time'].sum()
            title = '{}: {} \n Time vs {} | g={} | time={}'.format(env['readable_name'],
                                                         problem_name_to_descriptive_name(problem_name),
                                                         step_term,
                                                         gamma,
                                                         round(total_time, 3))
            file_name = '{}/{}/{}_time_{}.png'.format(base_dir, problem_name, mdp, gamma)
            ...
            title = '{}: {} \n {} and Delta vs {} | g={} | time={}'.format(env['readable_name'],
                                                          problem_name_to_descriptive_name(problem_name),
                                                          reward_term, step_term, gamma, round(total_time, 3))
            file_name = '{}/{}/{}_reward_delta_{}.png'.format(base_dir, problem_name, mdp, gamma)
            ...
            if problem_name == 'Q' and 'episode_file' in mdp_files:
                title = '{}: {} - {}'.format(env['readable_name'],
                                             problem_name_to_descriptive_name(problem_name),
                                             '{}')
