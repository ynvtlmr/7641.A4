...
from .plotting import plot_policy_map, plot_value_map, plot_value_grad, plot_value_q
import solvers
...
class ExperimentStats(object):
    ...
    def plot_q_on_map(self, file_name_base, map_desc, qs, direction_map):
        v = self.vs[-1].reshape(map_desc.shape)
        q_file_name = file_name_base.format('Q', 'Last')
        p = plot_value_q(v, qs._Q, map_desc, direction_map)
        p.savefig(q_file_name, format='png', dpi=150)
        p.close()
    ...
    def plot_policies_on_map(self, file_name_base, map_desc, color_map, direction_map, experiment, step_preamble,
        ...
        if only_last:
            ...
            grad_file_name = file_name_base.format('Grad', 'Last')
            ...
            p = plot_value_grad(v, map_desc)
            p.savefig(grad_file_name, format='png', dpi=150)
            p.close()
        else:
            l = len(self.policies)
            if step_size == 1 and l > 20:
