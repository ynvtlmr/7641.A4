...
class QLearningSolver(BaseSolver):
    def __init__(self, env, max_episodes, max_steps_per_episode=500, discount_factor=1.0, alpha=0.5, alpha_decay=0.001, alpha_min=0.1,
                 epsilon=0.1, epsilon_decay=0.001, epsilon_min=0.1, q_init=0, theta=0.0001, min_consecutive_sub_theta_episodes=10, verbose=False):
    ...
    def step(self):
        ...
        for t in range(self._max_steps_per_episode+1):
            ...
            if np.random.random() < self._epsilon:
                # random action
                action = np.random.randint(self._env.nA)
            else:
            ...
            # Decay alpha
            self._alpha -= self._alpha * self._alpha_decay
            self._alpha = max(self._alpha, self._alpha_min)
